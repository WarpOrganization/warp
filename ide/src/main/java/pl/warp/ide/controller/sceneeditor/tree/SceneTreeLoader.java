...
import pl.warp.engine.core.component.listenable.ListenableParent;
...
public class SceneTreeLoader {
    ...
    public SceneTreeLoader(ItemLookRepository descRepository) {
        this.descRepository = descRepository;
    }

    public void loadScene(GameScene scene, TreeView<GameComponent> sceneTree, SceneEditorController sceneEditorController) {
        sceneTree.setCellFactory(param -> new ComponentCell(descRepository, sceneEditorController));
        ...
        scheduledExecutor.schedule(() -> {
            if (sceneChanged) {
                sceneChanged = false;
                Platform.runLater(() -> loadScene(scene, sceneTree, sceneEditorController));
            }
