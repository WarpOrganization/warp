 ...
 */
public class PlanetProgram extends MeshRendererProgram {
    ...
    public void useComponent(Component component) {
        if (component.hasEnabledProperty(PlanetProperty.PLANET_PROPERTY_NAME)) {
            PlanetProperty property = component.getProperty(PlanetProperty.PLANET_PROPERTY_NAME);
            useTexture(property.getPlanetSurfaceTexture(), SURFACE_TEXTURE_SAMPLER);
        } else throw new IllegalArgumentException("Component needs an enabled planet property in order to render a gasplanet planet.");
    }
