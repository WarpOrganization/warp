...
uint packUnsignedNorm(float f, int bits) {
    ...
    return uint(round(clamp(f, 0.0, 1.0) * (0x01 << (bits - 1)))) & nBitMask(ubits);
}


float unpackUnsignedNorm(uint i, int bits) {
    uint ubits = uint(bits);
    return clamp((i & nBitMask(ubits)) / ((0x01 << (bits - 1)) * 1.0), 0.0, 1.0);
}

uint packSignedNorm(float f, int bits) {
    uint ubits = uint(bits);
    uint sign = (f < 0) ? 0 : 1;
    uint i = uint(round(clamp(f, -1.0, 1.0) * (0x01 << (bits - 2)))) & nBitMask(ubits - 1);
    return ((sign == 0) ? nBitMask(ubits - 1) ^ i : i) | (sign << (bits - 1));
}

float unpackSignedNorm(uint i, int bits) {
    uint ubits = uint(bits);
    float f = clamp((i & nBitMask(ubits - 1)) / ((0x01 << (bits - 2)) * 1.0), -1.0, 1.0);
